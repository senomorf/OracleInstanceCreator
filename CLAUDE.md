...
- **Multi-AD Cycling**: Each shape can independently cycle through multiple availability domains
...
### Combined Strategy: Parallel + Multi-AD Cycling

**ARCHITECTURAL BREAKTHROUGH**: The merged system combines both parallel execution and multi-AD cycling for maximum success rates:

**Implementation Flow:**
1. **launch-parallel.sh** launches both shapes simultaneously in background processes
2. Each shape calls **launch-instance.sh** with shape-specific environment variables
3. **launch-instance.sh** implements multi-AD cycling for each shape independently
4. Both shapes can cycle through multiple ADs (e.g., AD-1 → AD-2 → AD-3) in parallel

**Success Scenarios:**
- **Both shapes succeed in AD-1**: ~20-25 seconds total execution
- **One shape needs AD cycling**: ~30-40 seconds (still under 1-minute billing)
- **Both shapes cycle through multiple ADs**: ~45-55 seconds maximum (timeout protection)

**Benefits:**
- **Maximum Success Probability**: Each shape tries multiple ADs independently
- **Optimal Performance**: Parallel execution reduces total time
- **Billing Efficiency**: Single job stays under 1-minute billing threshold
- **Fault Tolerance**: One shape failing doesn't affect the other
